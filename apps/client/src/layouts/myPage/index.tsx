...
import { PCHeader, MobileHeader } from 'shared/components';
...
type item = Omit<DeliveryInfoType, 'isPreparing' | 'isArrive'>;
...
const MyPageLayout = () => {
  ...
  useEffect(() => {
    if (width > MobileWidth) setIsMobile(false);
    if (width < MobileWidth) setIsMobile(true);
  }, [width]);

  return (
    <>
      {isMobile ? <MobileHeader /> : <PCHeader type='client_black' />}
      <S.Container>
        <S.Wrapper>
          <S.TitleWrapper>
          ...
          <S.ContentsWrapper>
            {items.map((info: item, idx: number) => (
              <DeliveryInfo
                key={idx}
                isPreparing={true}
